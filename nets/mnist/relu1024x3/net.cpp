...
void HeBNN::init(FILE* in_file, bool b_prep)
{
	...
	conv_dep =  1024 ;
#if defined(GPU_ENC)
	layer2 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_RELU, &params);
#else
	layer2 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_RELU, &params, bk) ;
#endif

		//Layer 3
	conv_dep =  1024 ;
#if defined(GPU_ENC)
	layer3 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_RELU, &params);
#else
	layer3 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_RELU, &params, bk) ;
#endif
	...
	conv_dep =  10 ;
#if defined(GPU_ENC)
	layer4 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_NONE, &params);
#else
	layer4 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_NONE, &params, bk) ;
#endif
...
#endif
#if !defined(GPU_ENC)
	mbdata = (tMultiBit*)layer3->execute((tFixedPoint*) mbdata) ;
#else
	mbdata = (tMultiBitPacked*)layer3->execute((tMultiBitPacked*) mbdata) ;
#endif
#if !defined(GPU_ENC)
	mbdata = (tMultiBit*)layer4->execute((tFixedPoint*) mbdata) ;
#else
	mbdata = (tMultiBitPacked*)layer4->execute((tMultiBitPacked*) mbdata) ;
#endif
