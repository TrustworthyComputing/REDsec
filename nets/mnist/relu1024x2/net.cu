...
void HeBNN::init(FILE* in_file, bool b_prep)
{
	...
	conv_dep =  10 ;
#if defined(GPU_ENC)
	layer3 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_NONE, &params);
#else
	layer3 = new IntLayer(E_FC, conv_dep, E_NO_POOL, E_ACTIVATION_NONE, &params, bk) ;
#endif
...
void HeBNN::get_out_dims(tDimensions* out)
{
	if(out != NULL)
	{
		...
		memcpy(out, &(layer3->out_dim), sizeof(*out)) ;
